...
class QueryOperatorAnd extends QueryOperator {
	...
	matches(value, operatorValue, operator, query) {
		...
		return _.every(operatorValue, subquery => query._queryFactory.createQuery(subquery).matches(value) );
	}
...
class QueryOperatorOr extends QueryOperator {
	...
	matches(value, operatorValue, operator, query) {
		...
		return _.some(operatorValue, subquery => query._queryFactory.createQuery(subquery).matches(value) );
	}
