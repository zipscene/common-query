...
class Update extends EventEmitter {
	...
	normalize(options = {}) {
		...
		if (this.isFullReplace()) {
			if (options.schema) options.schema.normalizeSchema();
		} else {
			let updateOperators = this._updateFactory._updateOperators;

			for (let key in update) {
				let operatorValue = update[key];

				if (key[0] === '$') {
					if (key === '$set') {
						// handle $set
						console.warn('TODO: handle $set in updates');
					}
					...
					updateOperator.normalize(operatorValue, options, update, key);
					updateOperator.validate(operatorValue, key, options);
				} else {
					// This key is a field name
					update[key] = Update._normalizeValue(key, operatorValue, options);
				}
			}
		}
	}
	...
	static _normalizeValue(field, value, options = {}) {
		if (options.schema && value !== null && value !== undefined) {
			let subschema = options.schema._createSubschema(options.schema.getSubschemaData());
			let result;
			try {
