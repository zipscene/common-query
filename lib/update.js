...
class Update extends EventEmitter {
	 ...
	 * @since v1.3.0
	 */
	static createFromObjectDiff(from, to, options = {}) {
		let replaceEqualArrays = (
			options.replaceArrays === true ||
			options.replaceArrays === 'EQUAL'
		);
		let replaceDifferentArrays = (
			options.replaceArrays === true ||
			options.replaceArrays === 'DIFFERENT'
		);
		let replaceSmallerArrays = (
			options.replaceArrays === true ||
			options.replaceArrays === 'SMALLER' ||
			options.replaceArrays === 'DIFFERENT'
		);
		let replaceLargerArrays = (
			options.replaceArrays === true ||
		...
		const buildUpdateFromDiff = function(from, to, initialPath = '') {
			let key;
			...
			for (key in to) {
				...
				if (_.isArray(toValue)) {
					...
					if (
						(replaceEqualArrays && toValueLength === fromValueLength) ||
						(replaceDifferentArrays && toValueLength !== fromValueLength) ||
						(replaceLargerArrays && toValueLength > fromValueLength) ||
						(replaceSmallerArrays && toValueLength < fromValueLength)
					) {
						ensureCollection(patch, '$set');
						patch.$set[keyPath] = toValue;
						continue;
					...
					change = buildUpdateFromDiff(fromValue, toValue, keyPath);
