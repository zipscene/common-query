...
class Update extends EventEmitter {
	...
	normalize(options = {}) {
		...
		if (this.isFullReplace()) {
			...
			for (let operator in update) {
				...
				let operatorValue = update[operator];
				...
				if (options.schema) {
					if (operator[0] === '$') {
						...
						updateOperator.normalize(operatorValue, operator, update, options);
					} else {
						throw new UpdateValidationError('Updates may not contain a mix of operators and non-operators');
					}
				} else {
					updateOperator.normalize(operatorValue, operator, update, options);
				}
	...
	/**
	 * Create an update query from the diff of two objects
	 *
	 * @method createFromObjectDiff
	 * @static
	 * @param {Object} from
	 * @param {Object} to
	 * @param {Object} [options={}]
	 ...
	 * @param {String} [initialPath=''] - Initial path, used for recursive reference
	 * @return {Object} An update query to patch `from` to `to`
	 */
	static createFromObjectDiff(from, to, options = {}, initialPath = '') {
		let patch = {};

		let replaceEqualArrays = options.replaceArrays === true || options.replaceArrays === 'EQUAL';
		let replaceDifferentArrays = options.replaceArrays === true || options.replaceArrays === 'DIFFERENT';
		let replaceSmallerArrays = options.replaceArrays === true || options.replaceArrays === 'SMALLER' || options.replaceArrays === 'DIFFERENT';
		let replaceLargerArrays = options.replaceArrays === true || options.replaceArrays === 'LARGER' || options.replaceArrays === 'DIFFERENT';

		for (let key in from) {
			let keyPath = (initialPath) ? `${initialPath}.${key}` : key;

			if (!(key in to)) {
				ensureCollection(patch, '$unset');
			...
			if (key in from) {
				...
				if (_.isArray(toValue)) {
					if (_.isArray(fromValue)) {
						// If the situation matches `options.replaceArrays`,
						// replace arrays instead of recursively traversing them
						if (
                            (replaceEqualArrays && toValue.length === fromValue.length) ||
                            (replaceDifferentArrays && toValue.length !== fromValue.length) ||
                            (replaceLargerArrays && toValue.length > fromValue.length) ||
							(replaceSmallerArrays && toValue.length < fromValue.length)
						) {
							ensureCollection(patch, '$set');
						...
						if (fromValue.length > toValue.length) {
							ensureCollection(patch, '$push');
							patch.$push[keyPath] = {
								$slice: toValue.length
							};
						}

						let change = Update.createFromObjectDiff(fromValue, toValue, options, keyPath);
					...
					let change = Update.createFromObjectDiff(fromValue, toValue, options, keyPath);
					_.merge(patch, change);
				} else if (!_.isEqual(fromValue, toValue)) {
					// Overwrite old value
					ensureCollection(patch, '$set');
					patch.$set[keyPath] = toValue;
				}
...
/**
 * Ensure that a collection exists at a given key
 *
 * @function ensureCollection
 * @param {Object} collection
 * @param {String} key
 * @param {Boolean} makeArray - Whether to make the value an array instead of object
 * @return {Boolean} - Whether the item was already a collection
 */
function ensureCollection(parent, key, makeArray = false) {
	let isAlreadyPresent = _.isObject(parent[key]);
	if (!isAlreadyPresent) {
		if (makeArray === true) {
			parent[key] = [];
		} else {
