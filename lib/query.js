...
class Query {
	 ...
	 * to the path.
	 */
	static _getQueryPathSubschema(obj, schema, path) {
		// Edge case with root path
		if (path === '') {
			return [ schema.getData(), '' ];
		}
		return Query._getQueryPathSubschema(schema.getData(), path.split('.'), 0, '', schema);
	}
	...
	static _getQueryPathSubschemaHelper(subschema, pathArray, pathIdx, resultPath, schema) {
		...
		if (!subschema) {
			...
			if (isNaN(pathComponent)) {
				// Implied array
				return Query._getSchemaObjectPathHelper(
					subschema.elements,
					pathArray,
					pathIdx,
