...
class Query {
	...
	matches(value) {
		...
		// Since a path can actually math multiple paths throughout an object (via variable array matching), this will
		// be used to recursively check all possibly matching paths. Returns true or false, for whether or not
		// the path and expression value match against the object.
		let checkPath = (toMatch, pathArray, exprValue) => {
			let curToMatch = toMatch;
			for (let i = 0; i < pathArray.length; i++) {
				let prop = pathArray[i];
				if (prop === '') continue;
				if (objtools.isScalar(curToMatch)) return false;
				if (Array.isArray(curToMatch)) {
					// Check the remaining path against each array element
					for (let elem of curToMatch) {
						if (checkPath(elem, pathArray.slice(i), exprValue)) {
							return true;
						}
					}
					// If the current property is numeric, it can also be used to match a particular array element
					if (/^[0-9]+$/.test(prop)) {
						let arrIndex = parseInt(prop);
						if (arrIndex < curToMatch.length) {
							if (checkPath(curToMatch[arrIndex], pathArray.slice(i+1), exprValue)) {
								return true;
							}
						}
					}
					...
					// Normal object match
					curToMatch = curToMatch[prop];
				}
			}
			return this._expressionValueMatches(curToMatch, exprValue);
		};
		...
		for (let key in query) {
			...
			if (key[0] === '$') {
				...
				if (!checkPath(value, key.split('.'), exprValue)) {
					return false;
				}
			}
		}
