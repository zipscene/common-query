...
class Query {
	...
	/**
	 * Gets the value at a path in an object, using a schema for hints on how to traverse
	 * the object.  This is similar to objtools.getPath(), but this will also handle query
	 * paths that match arrays.  Without a schema, these matches can match multiple
	 ...
	 *
	 * @method _getSchemaObjectPath
	 * @private
	 * @throws {ObjectMatchError} - If the path doesn't correspond to a valid path in the schema.
	 * @param {Mixed} obj - The object to get the path in.
	 * @param {Schema} schema - The CommonSchema instance on the schema.
	 * @param {String} path - The path to search for.
	 * @return {Array} - Returns [ result, resultPath, subschema ] where result is the value
	 * in the object at the path, resultPath is the same as the input `path` parameter but with
	 * additional `$` keys added for implied arrays, and subschema is the subschema corresponding
	 * to the path.
	 */
	_getSchemaObjectPath(obj, schema, path) {
		// Edge case with root path
		if (path === '') {
			return [ obj, '', schema.getData() ];
		}
		return _getSchemaObjectPathHelper(obj, schema.getData(), path.split('.'), 0);
	}

	_getSchemaObjectPathHelper(value, subschema, pathArray, pathIdx) {
	 ...
	 */
	_expressionMatchesPath(matchValue, exprValue, pathArray, pathComponentStartIdx = 0) {
		...
		for (let parentIdx = pathComponentStartIdx; parentIdx <= pathArray.length; parentIdx++) {
			if (Array.isArray(currentParent)) {
				...
				for (let elem of currentParent) {
					...
					try {
						if (this._expressionMatchesPath(elem, exprValue, pathArray, parentIdx)) {
							return true;
						}
					} catch (ex) {
		...
		if (this._expressionValueMatches(currentParent, exprValue)) {
			return true;
		}
		return false;
	}
	...
	matches(value) {
		...
		for (let key in query) {
			...
			if (key[0] === '$') {
				...
				if (!this._expressionMatchesPath(value, exprValue, key.split('.'))) {
					return false;
				}
			}
		}
