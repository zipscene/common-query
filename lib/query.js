...
class Query {
	 ...
	 *
	 * @method normalize
	 * @throws {ValidationError} - If a query value cannot be normalized.
	 * @param {Object} [options]
	...
	normalize(options = {}) {
		...
		this._traverse({
			queryOperator(exprValue, operator, query) {
				...
				queryOperator.normalize(exprValue, operator, options, query);
			},
			exprOperator(exprValue, field, operator, expr, query) {
				let exprOperator = exprOperators[operator];
				exprOperator.normalize(exprValue, field, operator, expr, options, query);
			},
			exactMatch: _.noop,
			query(query) {
				query = query.getData();
