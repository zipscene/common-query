...
const ExprOperator = require('./expr-operator');
const Query = require('./query');
const XError = require('xerror');
const QueryValidationError = require('./query-validation-error');
const ObjectMatchError = require('../object-match-error');
// geolib is used when a distance conversion to meters is required, and jsts in other instances
...
function validateGeojson(geojson, allowedTypes) {
	if (!jsts) {
		throw new XError(XError.UNSUPPORTED_OPERATION, 'jsts is not available');
	}
	let geoReader = new jsts.io.GeoJSONReader();
	let geometry;
	try {
		geometry = geoReader.read(geojson);
	} catch (err) {
		throw new QueryValidationError('Geometry is invalid GeoJSON');
	}
	if (!geometry.isValid()) {
		throw new QueryValidationError('Geometry is not topologically valid');
	}
	if (allowedTypes && allowedTypes.indexOf(geometry.getGeometryType()) === -1) {
		throw new QueryValidationError('Geometry has invalid GeoJSON type');
	}
}
...
function normalizeGeojsonCoords(loc) {
	if (Array.isArray(loc)) {
		if (typeof loc[0] !== 'number' && typeof +loc[0] === 'number') loc[0] = +loc[0];
		if (typeof loc[1] !== 'number' && typeof +loc[1] === 'number') loc[1] = +loc[1];
	}
}
...
function isGeojsonCoordinateArray(value) {
	if (!Array.isArray(value)) {
		...
		return true;
	} else if (objtools.isPlainObject(value[0])) {
...
class ExprOperatorNear extends ExprOperator {
	...
	matches(value, operatorValue, operator, expr, options, query) {
		...
		if (operatorValue.$minDistance !== undefined) {
			if (typeof operatorValue.$minDistance !== 'number') {
		...
		if (operatorValue.$maxDistance !== undefined) {
			if (typeof operatorValue.$maxDistance !== 'number') {
	...
	validate(operatorValue) {
		...
		if (operatorValue.$minDistance !== undefined) {
			if (typeof operatorValue.$minDistance !== 'number' || isNaN(operatorValue.$maxDistance)) {
				throw new QueryValidationError('$minDistance must be a number');
			}
		}
		if (operatorValue.$maxDistance !== undefined) {
			if (typeof operatorValue.$maxDistance !== 'number' || isNaN(operatorValue.$maxDistance)) {
				throw new QueryValidationError('$maxDistance must be a number');
			}
		}
	}
...
class ExprOperatorIntersects extends ExprOperator {
	...
	matchesValue(value, operatorValue) {
		let geoReader = new jsts.io.GeoJSONReader();
		let operatorGeo, valueGeo;
		try {
			operatorGeo = geoReader.read(operatorValue.$geometry);
			valueGeo = geoReader.read(value);
		} catch (err) {
			throw new QueryValidationError('Geometry is invalid GeoJSON');
		}
		return operatorGeo.intersects(valueGeo);
	}

	validate(operatorValue) {
		if (!operatorValue || !operatorValue.$geometry) {
			throw new QueryValidationError('Geo operator must have $geometry key');
		}
		validateGeojson(operatorValue.$geometry);
	}
