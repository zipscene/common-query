...
class Query {
	 ...
	 */
	condense() {
		function includesEmptyOr(array) {
			return _.find(array, (elem) => _.isEqual(elem, { $or: [] })) !== undefined;
		}

		function traverse(value, operator, parent, parentKey, grandParent) {
			if (operator === '$and' && includesEmptyOr(value)) {
				delete parent[operator];
				parent.$or = [];
				return;
			}

			if (operator === '$nor' && includesEmptyOr(value)) {
				parent[operator] = _.reject(value, (elem) => _.isEqual(elem, { $or: [] }));
			}
