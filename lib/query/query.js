...
class Query {
	 ...
	 */
	condense() {
		function includesContradiction(array) {
			for (let elem of array) {
				if (isContradiction(elem)) return true;
			}
			return false;
		}

		function isContradiction(obj) {
			return (hasEmptyOr(obj) || hasNorWithEmptyObject(obj) || hasAndWithContradiction(obj));
		}

		function hasEmptyOr(obj) {
			return (_.isObject(obj) && obj.$or !== undefined && isEmptyArray(obj.$or));
		}
		...
		function hasNorWithEmptyObject(obj) {
			return (_.isObject(obj) && obj.$nor !== undefined && containsEmptyObject(obj.$nor));
		}

		function containsEmptyObject(arr) {
			for (let elem of arr) {
				if (isEmptyObject(elem)) { return true; }
			}
			return false;
		}

		function isEmptyObject(obj) {
			return (_.isObject(obj) && _.size(obj) === 0);
		}

		function hasAndWithContradiction(obj) {
			return (_.isObject(obj) && obj.$and !== undefined && includesContradiction(obj.$and));
		}

		function mutateToMinimalContradiction(obj) {
			for (let key in obj) {
				delete obj[key];
			}
			obj.$nor = [ {} ];
		...
		function traverse(value, operator, parent, parentKey, grandParent) {
			...
			if (isContradiction(parent)) {
				mutateToMinimalContradiction(parent);
				return;
			}
			// strip contradictions from $or and $nor
			if (_.includes([ '$or', '$nor' ], operator) && includesContradiction(value)) {
				let newValue = [];
			...
			if (!_.isString(operator) || operator[0] !== '$') return;

			let combinedKeys = [];
			let combinedClauses = [];

			// Delete empty clauses
			if (_.contains([ '$and', '$nor' ], operator) && value.length === 0) {
				delete parent[operator];
				if (_.isEmpty(parent)) {
					if (_.isArray(grandParent)) _.pullAt(grandParent, parentKey);
					if (_.isPlainObject(grandParent)) delete grandParent[parentKey];
				}
			}

			// Combine single-element clauses
			if (_.contains([ '$and', '$or' ], operator) && value.length === 1) {
				...
				for (let key in clause) {
					...
					if (_.contains(combinedKeys, key)) {
	 ...
	 */
	_traverse(handlers) {
		if (!_.isFunction(handlers.queryOperator)) handlers.queryOperator = _.noop;
		if (!_.isFunction(handlers.exprOperator)) handlers.exprOperator = _.noop;
		if (!_.isFunction(handlers.exactMatch)) handlers.exactMatch = _.noop;
		if (!_.isFunction(handlers.query)) handlers.query = _.noop;
