...
class Query {
	...
	condense() {
		...
		if (data.$and && data.$and.length <= 1) {
			let clause = data.$and[0];

			if (_.size(_.keys(clause)) === 1 && clause.$and) {
				data.$and = clause.$and;
				if (data.$and.length > 1) return;
				clause = data.$and[0];
			}
			...
			for (let key in clause) {
				if (data[key]) {
					...
					combinedClauses.push({ [key]: data[key] });
					delete data[key];
				}
		...
		if (data.$or && data.$or.length <= 1) {
			let clause = data.$or[0];
			for (let key in clause) {
				if (data[key]) {
					combinedKeys.push(key);
					combinedClauses.push({ [key]: data[key] });
		...
		if (data.$nor && !data.$nor.length) delete data.$nor;

		if (combinedClauses.length) {
			if (!_.isArray(data.$and)) data.$and = [];
			data.$and = data.$and.concat(combinedClauses);
		}
	}
