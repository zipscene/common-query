...
class Query {
	...
	constructor(queryData, queryFactory, options = {}) {
		if (_.isFunction(queryData.getData)) queryData = objtools.deepCopy(queryData.getData());
	...
	_substituteVars(vars, ignoreMissing) {
		...
		function subst(obj) {
			if (Query.isVarExpression(obj)) {
				let varName = obj.$var;
				if (objtools.getPath(vars, varName)=== undefined) {
					if (ignoreMissing) {
						return null;
					} else {
					...
					return objtools.getPath(vars, varName);
				}
...
function isContradiction(obj) {
	return (
		...
		_.isObject(obj) && (
			// has empty $or
			(!_.isUndefined(obj.$or) && _.isEmpty(obj.$or)) ||
			// has $nor w/ tautology
			(!_.isUndefined(obj.$nor) && _.some(obj.$nor, isTautology)) ||
			// has $and w/ contradiction
			(!_.isUndefined(obj.$and) && _.some(obj.$and, isContradiction))
		)
	);
}
	...
	return (
		...
		_.isObject(obj) && (
			...
			(_.every(obj, isLogicalExpressionOperator) && (
				// $or w/ tautology
				(_.isUndefined(obj.$or) || _.some(obj.$or, isTautology)) &&
				// $and w/ all tautologies
				(_.isUndefined(obj.$and) || _.every(obj.$and, isTautology)) &&
				// $nor w/ all contradictions
				(_.isUndefined(obj.$nor) || _.every(obj.$nor, isContradiction))
			))
		)
	);
}
