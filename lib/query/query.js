...
function isContradiction(obj) {
	return (
		_.isObject(obj) && (
			// has empty $or
			(obj.$or !== undefined && _.isArray(obj.$or) && obj.$or.length === 0) ||
			// has $nor w/ {}
			(obj.$nor !== undefined && containsEmptyObject(obj.$nor)) ||
			// has $and w/ contradiction
			(obj.$and !== undefined && includesContradiction(obj.$and))
		)
	);
}
...
function containsEmptyObject(arr) {
	for (let elem of arr) {
		if (_.isObject(elem) && _.size(elem) === 0) return true;
	}
	return false;
}
