...
class ExprOperatorExists extends ExprOperator {
	...
	validate(operatorValue) {
		if (operatorValue !== true || operatorValue !== false) throw new QueryValidationError('Value of $exists must be boolean');
	}
...
class ExprOperatorNot extends ExprOperator {
	...
	validate(operatorValue) {
		if (!objtools.isPlainObject(operatorValue)) throw new QueryValidationError('Argument of $not must be an expression');
	}
...
class ExprOperatorSize extends ExprOperator {
	...
	normalize(operatorValue, field, operator, expr, options, query, parent, parentKey) {
		...
		if (typeof operatorValue === 'number' && ~~operatorValue !== operatorValue) operatorValue = Math.floor(operatorValue);
