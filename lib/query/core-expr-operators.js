...
class ExprOperatorSize extends ExprOperator {
	...
	normalize(operatorValue, field, operator, expr, options, query, parent, parentKey) {
		...
		if (_.isNumber(operatorValue) && operatorValue % 1 > 0) operatorValue = Math.floor(operatorValue);
		parent[parentKey] = operatorValue;
	}
	...
	validate(operatorValue) {
		...
		if (operatorValue % 1 > 0) throw new QueryValidationError(`Argument to ${this.getName()} must be integer`);
	}
}
