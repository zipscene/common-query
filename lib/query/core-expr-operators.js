/* eslint-disable max-params, spaced-comment, prefer-template, space-infix-ops, no-bitwise */
...
class ExprOperatorExists extends ExprOperator {
	...
	validate(operatorValue) {
		if (operatorValue !== true && operatorValue !== false) {
			throw new QueryValidationError('Value of $exists must be boolean');
		}
	}
...
class ExprOperatorNot extends ExprOperator {
	...
	validate(operatorValue) {
		if (!objtools.isPlainObject(operatorValue)) {
			throw new QueryValidationError('Argument of $not must be an expression');
		}
	}
...
class ExprOperatorSize extends ExprOperator {
	...
	normalize(operatorValue, field, operator, expr, options, query, parent, parentKey) {
		if (typeof operatorValue === 'string') operatorValue = parseInt(operatorValue, 10);
		if (typeof operatorValue === 'number' && ~~operatorValue !== operatorValue) {
			operatorValue = Math.floor(operatorValue);
		}
	...
	validate(operatorValue) {
		if (!operatorValue || typeof operatorValue !== 'number' || isNaN(operatorValue)) {
...
class ExprOperatorText extends ExprOperator {
	...
	validate(operatorValue) {
		if (typeof operatorValue !== 'string') throw new QueryValidationError('Argument to $text must be string');
	}
...
class ExprOperatorWildcard extends ExprOperator {
	...
	validate(operatorValue) {
		if (typeof operatorValue !== 'string') throw new QueryValidationError('Argument to $wildcard must be string');
	}
...
class ExprOperatorRegex extends ExprOperator {
	...
	validate(operatorValue) {
		if (typeof operatorValue !== 'string') throw new QueryValidationError('Argument to $regex must be string');
	}
