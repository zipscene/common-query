	...
	normalize(operatorValue, operator, update, options) {
		if (_.isPlainObject(operatorValue)) {
			for (let key in operatorValue) {
				...
				if (!_.isNaN(numericValue)) update[operator][key] = numericValue;
			}
		} else {
			this.validate(operatorValue, operator, update);
		}
...
class UpdateOperatorAddToSet extends UpdateOperator {
	...
	normalize(operatorValue, operator, update, options) {
		if (_.isPlainObject(operatorValue)) {
			for (let key in operatorValue) {
				if (_.isPlainObject(operatorValue[key]) && _.isArray(operatorValue[key].$each)) {
					update[operator][key] = Update._normalizeValue(key, operatorValue[key].$each, options);
				} else {
