...
class ExprOperatorIn extends ExprOperator {
	...
	normalize(operatorValue, field, operator, expr, options, query, parent, parentKey) {
		if (_.isArray(operatorValue)) {
			for (let subOperator in operatorValue) {
				parent[parentKey][subOperator] = Query._normalizeValue(field, operatorValue[subOperator], options);
			}
		}
	}

	validate(operatorValue) {
		if (!_.isArray(operatorValue)) throw new QueryValidationError('Argument to $in must be array');
	}
...
class ExprOperatorNin extends ExprOperatorIn {
	...
	validate(operatorValue) {
		if (!_.isArray(operatorValue)) throw new QueryValidationError('Argument to $nin must be array');
	}
