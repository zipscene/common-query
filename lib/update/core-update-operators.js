...
class UpdateOperatorSet extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (updateData.$unset !== undefined && updateData.$unset !== null) {
			if (updateData.$unset[key]) delete updateData.$unset[key];
		}
...
class UpdateOperatorUnset extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (updateData.$set !== undefined && updateData.$set !== null) {
			if (updateData.$set[key]) delete updateData.$set[key];
		}
...
class UpdateOperatorInc extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue) {
...
class UpdateOperatorMin extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (
			(updateData.$max !== undefined && updateData.$max !== null) &&
			(updateData.$max[key] !== undefined && updateData.$max[key] !== null) &&
			composeOperatorValue[key] >= updateData.$max[key]
		) {
			delete updateData.$max[key];
			if (!Object.keys(updateData.$max).length) delete updateData.$max;
			if (!updateData.$set) updateData.$set = {};
			updateData.$set[key] = composeOperatorValue[key];
		} else {
			operatorValue[key] = composeOperatorValue[key];
		}
	}
...
class UpdateOperatorMax extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (
			(updateData.$min !== undefined && updateData.$min !== null) &&
			(updateData.$min[key] !== undefined && updateData.$min[key] !== null) &&
			composeOperatorValue[key] <= updateData.$min[key]
		) {
			delete updateData.$min[key];
			if (!Object.keys(updateData.$min).length) delete updateData.$min;
			if (!updateData.$set) updateData.$set = {};
			updateData.$set[key] = composeOperatorValue[key];
		} else {
			operatorValue[key] = composeOperatorValue[key];
...
class UpdateOperatorAddToSet extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue) {
		if (operatorValue[key] !== null && operatorValue[key] !== undefined) {
			let opValue;
			let composeValue;
			if (operatorValue[key].$each !== null && operatorValue[key].$each !== undefined) {
				opValue = operatorValue[key].$each;
			} else {
				opValue = [ operatorValue[key] ];
			}
			if (composeOperatorValue[key].$each !== null && composeOperatorValue[key].$each !== undefined) {
				composeValue = composeOperatorValue[key].$each;
			} else {
				composeValue = [ composeOperatorValue[key] ];
			}
...
class UpdateOperatorPush extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (
			(updateData.$pop !== undefined && updateData.$pop !== null) &&
			(updateData.$pop[key] !== undefined && updateData.$pop[key] !== null)
		) {
...
class UpdateOperatorPop extends UpdateOperator {
	...
	_computeUpdateValue(key, composeOperatorValue, operatorValue, updateData) {
		if (
			(updateData.$push !== undefined && updateData.$push !== null) &&
			(updateData.$push[key] !== undefined && updateData.$push[key] !== null)
		) {
			throw new ComposeUpdateError(`Cannot have a key that is both a $push and a $pop`, updateData);
		}

		if (
			(operatorValue[key] !== undefined && operatorValue[key] !== null) &&
			(composeOperatorValue[key] !== undefined && composeOperatorValue[key] !== null) &&
			composeOperatorValue[key] && composeOperatorValue[key] !== operatorValue[key]
		) {
