...
class UpdateOperatorMin extends UpdateOperator {
	...
	_composeUpdateValue(key, operatorValue, composeOperatorValue, updateData) {
		let keyValue = composeOperatorValue[key];
		if (operatorValue[key] && operatorValue[key] < keyValue) {
			keyValue = operatorValue[key];
		}
		if (updateData.$max && updateData.$max[key] && keyValue >= updateData.$max[key]) {
			delete updateData.$max[key];
			if (objtools.isEmptyObject(updateData.$max)) delete updateData.$max;
			if (!updateData.$set) updateData.$set = {};
			updateData.$set[key] = keyValue;
		} else {
			operatorValue[key] = keyValue;
		}
	}
...
class UpdateOperatorMax extends UpdateOperator {
	...
	_composeUpdateValue(key, operatorValue, composeOperatorValue, updateData) {
		if (updateData.$min && updateData.$min[key] && composeOperatorValue[key] <= updateData.$min[key])
		{
			delete updateData.$min[key];
			if (objtools.isEmptyObject(updateData.$min)) delete updateData.$min;
			if (!updateData.$set) updateData.$set = {};
...
class UpdateOperatorAddToSet extends UpdateOperator {
	...
	_composeUpdateValue(key, operatorValue, composeOperatorValue) {
		if (operatorValue[key]) {
			...
			let composeValue = composeOperatorValue[key].$each ? composeOperatorValue[key].$each : [ composeOperatorValue[key] ];
