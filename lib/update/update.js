...
class Update extends EventEmitter {
	...
	composeUpdate(update, options = {}) {
		...
		for (let operator in updateData) {
			let updateOperator = updateOperators[operator];
			if (!updateOperator) {
				throw new XError(XError.NOT_FOUND, `The update operator ${operator} does not exist`);
			}
			...
			if (Object.keys(mergedOperators).length) this._updateData[operator] = mergedOperators;
		}
	}
