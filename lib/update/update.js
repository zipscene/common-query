...
class Update extends EventEmitter {
	...
	static _buildUpdateFromDiff(from, to, patch, initialPath = '') {
		// There seems to be a rare mongo bug that, in rare cases, causes updates to paths inside of arrays
		// to corrupt the array and instead turn it into an object with numeric keys.  Enabling this workaround
		// causes the produced updates to always set the entire array.
		const enableMongoWorkaround = true;

		// Parameter sanity checks
		if (to === null || to === undefined) to = Array.isArray(from) ? [] : {};
		if (from === null || from === undefined) from = Array.isArray(to) ? [] : {};
		if (objtools.isScalar(from) || objtools.isScalar(to) || (!initialPath && Array.isArray(to))) {
		...
		if (Array.isArray(from) !== Array.isArray(to)) {
			...
			if ((arrayLengthChanged && elementsChanged) || (enableMongoWorkaround && (arrayLengthChanged || elementsChanged))) {
		...
		function syncValues(fromVal, toVal, path) {
			if (!objtools.isScalar(fromVal) && !objtools.isScalar(toVal)) {
