...
class Update extends EventEmitter {
	...
	getIncTrackingUpdate(trackingField) {
		let factory = this._updateFactory || commonQuery.defaultUpdateFactory;
		let trackingUpdateExpr = {};
		let incOperator = this._updateData.$inc;
		if (incOperator) {
			let $inc = trackingUpdateExpr.$inc = {};
			for (let field in incOperator) {
				let encodedField = field.replace(/\./g, '%');
				$inc[`${trackingField}.${encodedField}`] = -incOperator[field];
			}
		}
		return factory.createUpdate(trackingUpdateExpr);
	}
	...
	static _buildUpdateFromDiff(from, to, patch, options, initialPath = '') {
		...
		for (let key in to) {
			...
			} else if (objtools.isPlainObject(toValue)) {
				if (!objtools.isPlainObject(fromValue)) {
