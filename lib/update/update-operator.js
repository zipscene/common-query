...
class UpdateOperator {
	...
	compose(operatorValue, composeOperatorValue, updateData) {
		// Sanity checks
		if (
			composeOperatorValue !== undefined && composeOperatorValue !== null &&
			(operatorValue === null || operatorValue === undefined)
		) {
			return objtools.deepCopy(composeOperatorValue);
		}
		if (
			operatorValue === null || operatorValue === undefined ||
			composeOperatorValue === null || composeOperatorValue === undefined
		) {
			return operatorValue;
		}

		// A common case (as with $set, $inc, etc) is just to combine the objects.
		if (typeof operatorValue === 'object' && typeof composeOperatorValue === 'object') {
