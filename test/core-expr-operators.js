...
describe('Core Expression Operators', function() {
	describe('$exists', function() {
		it('$exists', function() {
			const query1 = createQuery({ foo: { $exists: true }, bar: { $exists: false } });
	...
	describe('$not', function() {
		it('negates an expression', function() {
			const query1 = createQuery({ foo: { $not: { $exists: true } }, bar: { $not: { $exists: false } } });
	...
	describe('$elemMatch', function() {
		it('$elemMatch', function() {
			const query1 = createQuery({ foo: { $elemMatch: { bar: 'baz', zip: 'buz' } } });
	...
	describe('$in', function() {
		it('$in', function() {
			const query1 = createQuery({ foo: { $in: [ 1, 2, 3 ] } });
	...
	describe('$nin', function() {
		it('$nin', function() {
			const query1 = createQuery({ foo: { $nin: [ 1, 2, 3 ] } });
	...
	describe('$regex', function() {
		it('$regex', function() {
			const query1 = createQuery({ foo: { $regex: '^zip.*zup.?$', $options: 'i' } });
			expect(query1.matches({ foo: 'zipasdzupa' })).to.be.true;
			expect(query1.matches({ foo: 'zipzup' })).to.be.true;
			expect(query1.matches({ foo: 'zipzupas' })).to.be.false;
			expect(query1.matches({ foo: 'zizup' })).to.be.false;
	...
	describe('$gt, $gte, $lt, $lte', function() {
		const query1 = createQuery({ foo: { $gt: 10, $lt: 20 }, bar: { $gte: 11, $lte: 19 } });
	...
	describe('$ne', function() {
		it('$ne', function() {
			const query1 = createQuery({ foo: { $ne: 'bar' } });
